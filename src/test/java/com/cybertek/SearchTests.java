...
public class SearchTests {
	...
	public void amazonSearchOne() throws InterruptedException {
		...
		String str = "Selenium Testing Tolls Cookbook";
		...
		try {
			// if the element is still in htm, this line will handle
			Assert.assertFalse(driver.findElement(By.xpath(xpath)).isDisplayed());
			
		} catch (NoSuchElementException e) {
			// if the element is not in html at all, exception will be thrown
			//since are verify sth does not exist, NoSuchElementException means test should pass
			e.printStackTrace();
		}
